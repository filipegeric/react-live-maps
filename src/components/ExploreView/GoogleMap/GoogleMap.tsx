...
const MapMarker: React.FC<{ color?: string; lat: number; lng: number }> = (props) => {
	return (
		<svg style={{ width: '50px', height: '50px', transform: 'scale(0.5) translateX(-100%) translateY(-150%)' }}>
			<path
				style={{ fill: props.color || 'red' }}
...
const GoogleMap: React.FC<{ events: Array<any> }> = (props) => {
	...
	return (
		<div className={`google-map column`} style={{ padding: 0, position: 'relative' }}>
			...
			>
				{props.events.map((el) => (
					<MapMarker key={el.id} lat={el.lat} lng={el.long} color={el.interest.color} />
				))}
			</GoogleMapReact>
